...
def process_tables(key, val, fmt, meta):
    ...
    """
    if key == 'Table':
        # val = [attr, caption, colspecs, head, body, foot]
        table_attr = val[0]
        table_id = table_attr[0]
        table_caption = val[1]

        if not table_id:
            return None

        # Standard class for tables
        t_class = 'table'
        if t_class in known_classes:
            known_classes[t_class] += 1
        else:
            known_classes[t_class] = 1

        known_ids[table_id] = str(known_classes[t_class])

        # Extract attributes for wrapping (parity with your image logic)
        keys = {k: v for k, v in table_attr[2]}

        if fmt in ['latex', 'pdf']:
            if 'wwidth' in keys:
                latex_size = keys['wwidth']
                latex_pos = keys.get('wpos', 'r')

                # We wrap the existing table structure in a wraptable environment
                # Note: This is a simplified approach using RawInlines
                begin_wrap = RawInline(fmt, "\\begin{{wraptable}}{{{pos}}}{{{size}}}\\centering".format(
                    pos=latex_pos, size=latex_size))
                end_wrap = RawInline(fmt, "\\label{{{id}}}\\end{{wraptable}}".format(id=table_id))

                # To actually wrap the table, we might need to return a list or modify the block
                # However, tables are Blocks, so we return a list of [Raw, Table, Raw]
                return [Para([begin_wrap]), Table(*val), Para([end_wrap])]

            # If not wrapping, Pandoc usually handles table labels via the ID attribute automatically.
            return None

        else:
            # Add "Table X. " to the caption for non-LaTeX formats
            label = [Strong([Str("Table ")])]
            number = [Strong([Str(known_ids[table_id])])]
            suffix = [Strong([Str(". ")])]

            # table_caption is [short_caption, [blocks]]
            # We modify the first block of the caption if it exists
            if table_caption[1]:
                content = table_caption[1][0]['c']
                table_caption[1][0]['c'] = label + number + suffix + content
